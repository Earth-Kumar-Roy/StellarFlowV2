...
import type { DisplayCurrency } from './Navbar';
import { 
  ShieldCheck, 
  UserCheck, 
  Clock, 
...
// Conversion rates relative to base tokens
const RATES: Record<string, { USD: number; EUR: number; INR: number }> = {
  XLM: { USD: 0.11, EUR: 0.10, INR: 9.15 },
  USDC: { USD: 1.00, EUR: 0.92, INR: 83.50 },
  EURC: { USD: 1.08, EUR: 1.00, INR: 90.20 },
};
...
export const EscrowCard: React.FC<EscrowCardProps> = ({
  ...
  useEffect(() => {
    ...
    if (saved) setCurrencyMode(saved);

    const handler = (e: any) => {
      if (e.detail) setCurrencyMode(e.detail);
    };
  ...
  const tokenSymbol = (escrow.currency || 'XLM').toUpperCase();
  const isMultiSigRequired = totalAmountNum > MULTISIG_THRESHOLD && tokenSymbol === 'XLM';

  // Accurately compute ACTUAL on-chain released funds
  ...
  const completionPercentage = totalAmountNum > 0 
    ? Math.min(100, Math.round((actualReleasedAmount / totalAmountNum) * 100))
    : 0;

  // Conversion formatting utility
  const formatAmount = (amountNum: number) => {
    if (currencyMode === 'NATIVE') {
      return { val: amountNum.toFixed(2), symbol: tokenSymbol };
    }
    const rateTable = RATES[tokenSymbol] || RATES.XLM;
    const rate = rateTable[currencyMode] || 1;
    const converted = amountNum * rate;

    const prefix = currencyMode === 'USD' ? '$' : currencyMode === 'EUR' ? '€' : '₹';
    return { val: `${prefix}${converted.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, symbol: currencyMode };
  };

  const totalFormatted = formatAmount(totalAmountNum);
  const releasedFormatted = formatAmount(actualReleasedAmount);
  ...
  // Copy structured audit summary
  const handleCopyAuditSummary = () => {
    const summaryText = `[StellarFlow Escrow Audit Report]
Vault Total: ${escrow.totalAmount} ${tokenSymbol} (${totalFormatted.val} ${totalFormatted.symbol})
Released: ${actualReleasedAmount} ${tokenSymbol} (${completionPercentage}% complete)
Client: ${escrow.client}
Freelancer: ${escrow.freelancer}
Milestones: ${escrow.milestones.length} defined
Status: ${activeStatus}
Expires: ${new Date(escrow.deadline * 1000).toUTCString()}`;
  ...
  return (
    <div className="relative bg-slate-900/80 backdrop-blur-xl border border-slate-800/90 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-indigo-950/20 max-w-4xl mx-auto my-8 overflow-hidden">
      ...
      <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800/80 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="flex items-center space-x-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium px-3 py-1.5 rounded-xl border border-slate-700 transition cursor-pointer"
            title="Copy structured escrow details"
          >
            {copiedAudit ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
            <span>{copiedAudit ? 'Copied' : 'Audit Summary'}</span>
          </button>

          {isFullyReleased && (
            <button
              onClick={() => setShowInvoiceModal(true)}
              className="flex items-center space-x-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold px-4 py-1.5 rounded-xl transition shadow-lg shadow-emerald-600/20 cursor-pointer"
            >
          ...
          <span
            className={`text-xs font-bold px-3.5 py-1 rounded-full border uppercase tracking-wider shadow-sm ${getStatusBadge(
              activeStatus
            )}`}
          >
            {activeStatus}
      ...
      {/* Real-Time Settlement Health & Progress Radar */}
      <div className="mt-6 p-4 rounded-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950/40 border border-slate-800/90 space-y-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs font-mono">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-slate-300 font-bold">Settlement Progress</span>
            <span className="text-slate-500">|</span>
            <span className="text-indigo-300 font-bold">{completionPercentage}% Disbursed</span>
          </div>

          <div className="flex items-center space-x-2">
            <span className="text-slate-400">Vault Health:</span>
            {timeLeft.isExpired && !isFullyReleased ? (
              <span className="text-rose-400 bg-rose-500/10 border border-rose-500/30 px-2 py-0.5 rounded text-[10px] font-bold">
                Expired (Refund Active)
              </span>
            ) : isFullyReleased ? (
              <span className="text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 rounded text-[10px] font-bold">
                100% Settled
              </span>
            ) : (
              <span className="text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 rounded text-[10px] font-bold">
        ...
        <div className="w-full bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-800">
          <div
            className="bg-gradient-to-r from-indigo-500 via-violet-500 to-emerald-400 h-full transition-all duration-500"
            style={{ width: `${completionPercentage}%` }}
          />
        </div>

        {/* Milestone Stepper Pills */}
        <div className="flex flex-wrap gap-1.5 pt-1">
          {escrow.milestones.map((m, idx) => {
            const isDone = m.isCompleted;
            const isSub = m.isSubmitted || m.isInReview;
            return (
              <span
                key={m.id || idx}
                className={`text-[10px] font-mono px-2.5 py-0.5 rounded-md border flex items-center space-x-1 ${
                  isDone
              ...
              >
                ...
                <span>
                  M{idx + 1}: {isDone ? 'Released' : isSub ? 'Review' : 'Pending'}
                </span>
              </span>
            );
          })}
      ...
      {/* Top Stat Cards Grid (With Multi-Currency Conversion) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
        <div className="bg-slate-800/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-700/80 transition-all">
          ...
          <p className="text-3xl font-black text-white font-mono mt-2 tracking-tight">
            {totalFormatted.val}{' '}
            <span className="text-sm font-semibold text-slate-400">{totalFormatted.symbol}</span>
          </p>
          {currencyMode !== 'NATIVE' && (
        ...
        <div className="bg-slate-800/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-700/80 transition-all">
          ...
          <p className="text-3xl font-black text-emerald-400 font-mono mt-2 tracking-tight">
            {releasedFormatted.val}{' '}
            <span className="text-sm font-semibold text-slate-400">{releasedFormatted.symbol}</span>
          </p>
          {currencyMode !== 'NATIVE' && (
            <p className="text-xs text-slate-500 font-mono mt-1">
              ≈ {actualReleasedAmount.toString()} {tokenSymbol}
            </p>
          )}
        </div>
      </div>
